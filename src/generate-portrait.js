...
export default async function handler(req, res) {
  ...
  try {
    // Pass image as base64 data URI directly — no separate upload needed
    ...
    console.log("Queue status:", queueResp.status, "body:", queueText.substring(0, 300));
    ...
    if (!request_id) return res.status(500).json({ error: "No request_id returned: " + queueText });

    console.log("Queued OK, request_id:", request_id);
